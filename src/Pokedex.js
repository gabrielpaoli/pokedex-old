								...
								return (
									<div key={pokemonInn.name} className="col m3">
										<Link to={`/pokedex/${this.parseId(pokemonInn.url)}`}>
											...
											<img className="image-100-responsive" src={`https://assets.pokemon.com/assets/cms2/img/pokedex/full/${this.pad(this.parseId(pokemonInn.url))}.png`} />												<p className="center-align">{pokemonInn.name}</p>
											</div>
										</Link>
									</div>
								);
